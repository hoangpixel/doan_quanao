...
public class ChiTietHoaDon extends javax.swing.JDialog {
     ...
     */
    public ChiTietHoaDon(java.awt.Frame parent, boolean modal, HoaDonDTO hd) {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(parent);
    
        // Lưu thông tin hóa đơn được truyền vào
        this.hd = hd;
    ...
    // Thêm phương thức để load danh sách CTHD
    private void loadCTHDData(int mahd) {
        try {
            ChiTietHoaDonBUS cthdBUS = new ChiTietHoaDonBUS();
            cthdBUS.docDSCTHD(); // Đọc toàn bộ CTHD từ database
        
            // Lọc ra các CTHD thuộc hóa đơn hiện tại
            ...
            System.out.println("Đã load " + dsCTHD.size() + " chi tiết hóa đơn của HD #" + mahd);
        } catch (Exception e) {
            System.out.println("Lỗi khi load chi tiết hóa đơn: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void btnXuatPDFActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXuatPDFActionPerformed
        try {
            ...
            fileChooser.setSelectedFile(new File("HoaDon_" + hd.getMahd() + ".pdf"));
            ...
            if (userSelection == JFileChooser.APPROVE_OPTION) {
                ...
                float titleWidth = fontBold.getStringWidth("CHI TIẾT HÓA ĐƠN") / 1000 * 18;
                ...
                contentStream.showText("Mã hóa đơn: " + tMaHD.getText());
                contentStream.endText();
                yPosition -= 20;
                ...
                contentStream.setFont(font, 12);
                contentStream.newLineAtOffset(margin, yPosition);
                contentStream.showText("Mã khách hàng: " + tMaKH.getText());
                contentStream.endText();
                yPosition -= 20;
                ...
                float tableHeight = (dsCTHD.size() + 1) * rowHeight; // +1 cho header
                ...
                for (int i = 0; i < dsCTHD.size(); i++) {
                    ...
                    String[] values = {
                        getTenByMaSP(item.getMaSanPham()),
                        getPhienBanInfo(item.getMaPhienBan()),
                    ...
                    if (i < dsCTHD.size() - 1 && yPosition - rowHeight < margin) {
                        contentStream.close();
                        page = new PDPage();
                        document.addPage(page);
                        contentStream = new PDPageContentStream(document, page);
    ...
    private void loadTableData() {
        ...
        for (int i = 0; i < dsCTHD.size(); i++) {
            ChiTietHoaDonDTO item = dsCTHD.get(i);
            String tenSP = getTenByMaSP(item.getMaSanPham());
            String phienBan = getPhienBanInfo(item.getMaPhienBan());
            data[i][0] = tenSP;
            data[i][1] = phienBan;
