...
public class SuaHoaDon extends javax.swing.JDialog {
     ...
     */
    public SuaHoaDon(java.awt.Frame parent, boolean modal, HoaDonDTO hd) {
        super(parent, modal);
        initComponents();
    ...
    tMaHD.setText(String.valueOf(hd.getMahd()));
    tMaNV.setText(String.valueOf(hd.getManv()));
    tMaKH.setText(String.valueOf(hd.getMakh()));
    tTongTien.setText(String.valueOf(hd.getTongtien()));
    cbxTrangThai.setSelectedIndex(hd.getTrangThai());
    
    // Thiết lập các control
    setupAutoCalculation();
    
    // Load chi tiết hóa đơn
    loadCTHDData(hd.getMahd());
    ...
    private void initComponents() {
        ...
        cbxPhienBan = new javax.swing.JComboBox<String>();
        ...
        cbxTrangThai.setModel(new javax.swing.DefaultComboBoxModel<String>(new String[] { "Chưa xử lí", "Đã xử lí" }));
    ...
    int kiemtra = cbxTrangThai.getSelectedIndex();
        if (kiemtra==1) {
            JLabel lbConfirm = new JLabel("Hóa đơn này sẽ không thể sửa hay xóa trong tương lai, bạn có chắc muốn thay đổi trạng thái thành đã xử lí chứ?");
    ...
    private void loadTableData() {
        ...
        for (int i = 0; i < dsCTHD.size(); i++) {
            ChiTietHoaDonDTO item = dsCTHD.get(i);
            data[i][0] = spbus.getTenSanPhamByMaSP(item.getMaSanPham());
            data[i][1] = pbspbus.getPhienBanByMaPB(item.getMaPhienBan());
    ...
    private void loadCTHDData(int mahd) {
        ChiTietHoaDonBUS cthdBUS = new ChiTietHoaDonBUS();
        cthdBUS.docDSCTHD(); // Đọc toàn bộ CTPN từ database
        
        // Lọc ra các CTPN thuộc phiếu nhập hiện tại
        for(ChiTietHoaDonDTO item : cthdBUS.ds) {
        ...
        System.out.println("Đã load " + dsCTHD.size() + " chi tiết hóa đơn");
    }
