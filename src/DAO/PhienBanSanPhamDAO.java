...
public class PhienBanSanPhamDAO {
    ...
    public int thayDoiSLPBTrongHD(int sl, int mapb) {
    int result = 0;
    String qry = "UPDATE phienbansp SET SLPB = SLPB - ? WHERE MAPB = ? AND SLPB >= ?";
    try (Connection con = DBConnect.getConnection();
         PreparedStatement pstm = con.prepareStatement(qry)) {
        pstm.setInt(1, sl);
        pstm.setInt(2, mapb);
        pstm.setInt(3, sl); // đảm bảo tồn kho hiện tại >= sl muốn trừ
        result = pstm.executeUpdate();
    } catch (Exception e) {
        e.printStackTrace();
    }
    ...
    public ArrayList<PhienBanSanPhamDTO> layPBSPTheoMaSP(int masp) {
        ...
        public int getTonKhoByMaPB(int maPB) {
        int tonKho = 0;
        String sql = "SELECT SLPB FROM phienbansp WHERE MAPB = ? AND is_deleted = 0 LIMIT 1";
